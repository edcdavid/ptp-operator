...
import (
	...
	"github.com/test-network-function/l2discovery/l2lib/pkg/nodes"
...
func GetL2DiscoveryConfig() (config *L2DiscoveryConfig, err error) {
	if GlobalL2DiscoveryConfig.refresh {
		err := GlobalL2DiscoveryConfig.DiscoverL2Connectivity(clientsholder.Client)
		if err != nil {
...
func (config *L2DiscoveryConfig) DiscoverL2Connectivity(aClient *clientsholder.ClientSet) error {
	...
	config.ptpInterfaces, err = buildPtpIfaces(aClient)
	if err != nil {
...
func buildPtpIfaces(aClient *clientsholder.ClientSet) (ptpIfaces map[string]map[string]bool, err error) {
	nodeDevicesList, err := aClient.NodePtpDevices(clientsholder.PtpLinuxDaemonNamespace).List(context.Background(), metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
