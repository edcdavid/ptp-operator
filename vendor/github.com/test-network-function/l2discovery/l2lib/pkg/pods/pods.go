...
import (
	...
	"github.com/test-network-function/l2discovery/l2lib/pkg/clientsholder"
...
func GetLog(p *corev1.Pod, containerName string) (string, error) {
	...
	req := clientsHolder.K8sClient.CoreV1().Pods(p.Namespace).GetLogs(p.Name, &corev1.PodLogOptions{Container: containerName})
...
func PodRole(runningPod *corev1.Pod, label string) (bool, error) {
	clientsHolder := clientsholder.GetClientsHolder()
	nodeList, err := clientsHolder.K8sClient.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{
		LabelSelector: label,
	})
	if err != nil {
