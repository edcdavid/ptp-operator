...
// Object representing a ethernet interface
type Iface struct {
	// interface name
	IfName string
...
// Object representing the L2 neighbors reacheable via a local Ethernet interface
type Neighbors struct {
	// Local Ethernet interface
	Local Iface
	// List of remote MACs reacheable from this interface
...
type L2DiscoveryConfig struct {
	...
	DiscoveryMap map[string]map[string]map[string]*Neighbors
...
func SameNic(ifaceName1, ifaceName2 *PtpIf) bool {
	...
	nic1, _ := getNicPort(ifaceName1.IfName)
	nic2, _ := getNicPort(ifaceName2.IfName)
	return nic1 == nic2
}

// gets the NIC port based on interface name (last numeral)
func getNicPort(ifaceName string) (nic, port string) {
	r := regexp.MustCompile(`(\d+)$`)
	port = r.FindString(ifaceName)
	nic = strings.TrimSuffix(ifaceName, port)
	return nic, port
}

// Gets the latest topology reports from the l2discovery pods
func (config *L2DiscoveryConfig) getL2Disc() error {
	config.DiscoveryMap = make(map[string]map[string]map[string]*Neighbors)
	index := 0
	for _, aPod := range config.L2DiscoveryPods {
		...
		var discDataPerNode map[string]map[string]*Neighbors
		if err := json.Unmarshal([]byte(report), &discDataPerNode); err != nil {
			return err
		}

		if _, ok := config.DiscoveryMap[aPod.Spec.NodeName]; !ok {
			config.DiscoveryMap[aPod.Spec.NodeName] = make(map[string]map[string]*Neighbors)
		}
...
func (config *L2DiscoveryConfig) createMaps(disc map[string]map[string]*Neighbors, nodeName string, index *int) {
	for _, ifaceData := range disc[experimentalEthertype] {
		...
		aIf := PtpIf{}
		aIf.NodeName = nodeName
		aIf.IfName = ifaceData.Local.IfName
		aIf.MacAddress = ifaceData.Local.IfMac.Data
		config.PtpIfList = append(config.PtpIfList, &aIf)
		(*index)++
	}
}
