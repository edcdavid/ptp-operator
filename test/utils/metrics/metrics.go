...
func InitMaxOffsetConfig() error {
	value, isSet := os.LookupEnv("MAX_OFFSET_IN_NS")
	if !isSet {
		logrus.Infof("MaxOffsetNs not set, assuming %d ns", MaxOffsetDefaultNs)
		MaxOffsetNs = MaxOffsetDefaultNs
		return nil
	}
	value = strings.ToLower(value)
	MaxOffsetNs, err := strconv.Atoi(value)
	if err != nil {
		return fmt.Errorf("cannot parse MaxOffsetNs, got %s, err:%s", value, err)
	}

	logrus.Infof("MaxOffsetNs=%d", MaxOffsetNs)
	return nil
}
